...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, input_dim)  # Map back to input_dim
        )

    def forward(self, x):
        return x + self.fc(x)  # Add input (residual connection)

class ResidualDiffusionModel1D(nn.Module):
    def __init__(self, input_dim, hidden_dim, time_embed_dim, num_residual_blocks=3):
        super(ResidualDiffusionModel1D, self).__init__()
...
if __name__ == '__main__':
    from torchinfo import summary  # Install via `pip install torchinfo`
    from fvcore.nn import FlopCountAnalysis  # Install via `pip install fvcore`

    model = ResidualDiffusionModel1D(1, 512, 512, 15)
    batch_size = 1
    print("Model Summary:")
    summary(model, input_size=[(batch_size, 1), (batch_size, 1)])

    # 2. Calculate and print FLOPs
    print("\nFLOPs:")
    x_t = t = torch.randn(1, 1).cuda()

    flops = FlopCountAnalysis(model, (x_t, t))
